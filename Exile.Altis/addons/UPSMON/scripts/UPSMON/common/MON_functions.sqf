	...
	_cargo = [];

  	{
		...
		if ( _vehgrpid == _grpid || (_emptypositions > 0 && _any)) then {
			...
			_locked = _vehicle getVariable ["ExileIsLocked",1];
			if ( ((count _unitsIn) > 0) && (_locked == 0)) then {
...
MON_NearestLandTransport = {
...

	{
		...
		if (canMove _x && _transportSoldier >= count (units _npc) && !(typeof _x in MON_bugged_vehicles)
...
MON_NearestsLandTransports = {
...

	{
		...
		if (_emptypositions > 0 && canMove _x && !(typeof _x in MON_bugged_vehicles)
...
MON_NearestsLandCombat = {
...

	{
		...
		if (_Gunnercount > 0 && canMove _x && !(typeof _x in MON_bugged_vehicles)
			&& (_drivercount > 0 || side _npc == side _x )) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
	}foreach _OCercanos;
...
MON_NearestsStaticWeapons = {
	...
	_distance = _this select 1;
...

		{
			...
			if (alive _x && _emptypositions > 0 && !(typeof _x in MON_bugged_vehicles) ) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
...
MON_Nearestsboats = {
	...
	_distance = _this select 1;
...

		{
			...
			if (_emptypositions > 0 && canMove _x && (_drivercount > 0 || side _npc == side _x )) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
