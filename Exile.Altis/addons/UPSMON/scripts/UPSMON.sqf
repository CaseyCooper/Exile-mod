...
while {_loop} do {
	...
	if ( _isSoldier && !_exit) then {
		...
		if (_pursue) then {
			...
			waitUntil {(count KRON_targetsPos > 0)};
			{
...
//		if (!_nowp && {alive _npc} && {canmove _npc} && {_wptype == "MOVE"} && {_timeontarget >= 60}) then {
	...
	//If in helicopter calculations must done faster
	...
	_cont = format ["KRON_UPS_%1",_npcname];

    if(SAR_EXTREME_DEBUG) then {diag_log format ["_cont = %1 for leader: %2", _cont, format ["KRON_UPS_%1",_npcname]];};

	if (_cont == "KRON_UPS_0") then {
		_exit=true;
	};
       ...
       //[nil, _x, "per", rHideObject, true] call RE;
        ...
        _i=0;
        {
            ...
            if (_i>1) then {
                ...
                if(_is_veh_group) then {
					[_newunit] spawn SAR_AI_trace_veh;
					_newunit setIdentity "id_SAR";
					[_newunit] spawn SAR_AI_reammo;
                } else {
					[_newunit] spawn SAR_AI_trace;
					_newunit setIdentity "id_SAR";
					[_newunit] spawn SAR_AI_reammo;
                };
