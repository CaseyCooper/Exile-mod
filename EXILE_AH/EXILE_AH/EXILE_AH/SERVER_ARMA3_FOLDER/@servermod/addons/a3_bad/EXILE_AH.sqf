...
baddieVERSION='25082015-v0001';
diag_log format['<baddie> %1 - STARTING',time];
...
FNC_A3_LOG_RAND_VAR = compileFinal "
	...
	diag_log ('<baddie>RandomVarLog| '+_this+' (v0001)');
";
_allRandomGenVars = [];
_counts = [];
{
...
_fnc_RandomGen =
{
	...
	_gen
};diag_log format['<baddie> _fnc_RandomGen: %1',_fnc_RandomGen];
format['----START-LINE----   (%1)',baddieVERSION] call FNC_A3_LOG_RAND_VAR;
...
format['----END-LINE----   (%1)',baddieVERSION] call FNC_A3_LOG_RAND_VAR;
...
diag_log format['<baddie> %1 - loading AntiHack..',time];
...
[
	...
	'dmcheats','kichdm','_news_banner','fucked up','lystics menu','rsccombo_2100','\dll\datmalloc','rsclistbox_1501',
...
'gearDialog_create','lystoKeypress','ThirtySix','LY_SwaggerLikeUs','Jkeyszz','n2','boxofmagic','MainScripts','DMC_fnc_4danews','baddieBYPASS','EXEC_TEXT','vehicle_dblclick','init_main',
...
[
	...
	'fnc_baddie_A3noRecoil','VEHBOOST_FUNCTION'
];
...
FNC_A3_HACKLOG = compileFinal "
	...
	diag_log ('<baddie>HL| '+_this+' (v0001)');
";
...
fnc_call_ARMALOAD = compileFinal "
	...
	if(_option == 0)exitWith
	{
		...
		if(_url in ARMALOAD_urlARRAY)exitWith{diag_log '<baddie> terminated double ARMA_LOAD call';};
...
diag_log format['<baddie> %1 - Thread BEFORE MAIN: compiling now!',time];
_A3AHstring = "
	diag_log format['<baddie> %1 - Thread BEFORE MAIN: still compiling...',time];
	...
	_server_setToken={
		...
		if(isNil'_input')exitWith{diag_log '<baddie> server_setToken: _input is NIL';};
		...
		if(_inputUID isEqualTo '')exitWith{diag_log '<baddie> server_setToken: _inputUID is EMPTY';};
		...
		if(isNull _inputOBJECT)exitWith{diag_log format['<baddie> server_setToken: (%1) _inputOBJECT is NULL',_inputUID];};
		
		[_token,(owner _inputOBJECT),(name _inputOBJECT),_inputUID] call "+_fnc_pvc_token+";
		diag_log format['<baddie> server_setToken: SENT TOKEN [%1] TO %2(%3)',_token,name _inputOBJECT,_inputUID];
	};
	...
	_RequestTokenCODE =
	{
		...
		_input = _this select 1;
		if(isNil '_input')exitWith{diag_log '<baddie> RequestToken: _input is Nil';};
		
		_clientUID = _input select 1;
		if(isNil '_clientUID')exitWith{diag_log '<baddie> RequestToken: _clientUID is Nil';};
		if!(typeName _clientUID isEqualTo 'STRING')exitWith{diag_log format['<baddie> RequestToken: typeName _clientUID != OBJECT -> %1',typeName _clientUID];};
		if(_clientUID isEqualTo '')exitWith{diag_log '<baddie> RequestToken: resolving _playerObj with UID not possible - no UID found';};
		
		_playerObj = _input select 0;
		if(isNil '_playerObj')exitWith{diag_log '<baddie> RequestToken: _playerObj is Nil';};
		if!(typeName _playerObj isEqualTo 'OBJECT')exitWith{diag_log format['<baddie> RequestToken: typeName _playerObj != OBJECT -> %1',typeName _playerObj];};
		if(isNull _playerObj)then
		{
			diag_log format['<baddie> RequestToken: resolving _playerObj of UID %1 via playableUnits',_clientUID];
			{
		...
		if(isNull _playerObj)exitWith{diag_log format['<baddie> RequestToken: %1(%2) _playerObj is NULL-OBJ',_playerObj,_clientUID];};
		
		_clientID = (owner _playerObj);
		if(isNil '_clientID')exitWith{diag_log format['<baddie> RequestToken: %1(%2) _clientID is Nil',name _playerObj,_clientUID];};
		
		_clientUIDcheck = getPlayerUID _playerObj;
		if(isNil '_clientUIDcheck')exitWith{diag_log format['<baddie> RequestToken: %1(%2) _clientUIDcheck is Nil',name _playerObj,_clientUID];};
		if!(_clientUID isEqualTo _clientUIDcheck)exitWith{diag_log format['<baddie> RequestToken: %1(%2) _clientUID != _clientUIDcheck  %2/%3',name _playerObj,_clientUID,_clientUIDcheck];};
		
		diag_log format['<baddie> RequestToken: %1(%2) Requested a Token!   _clientID: %3',name _playerObj,_clientUID,_clientID];
		if(_clientUID in "+_BigTokenArray+")then
		{
			_token = "+_BigTokenArray+" select (("+_BigTokenArray+" find _clientUID)+1);
			if(isNil '_token')exitWith{diag_log format['<baddie> RequestToken: %1(%2) _token is Nil',name _playerObj,_clientUID];};
			[_token,_clientID,(name _playerObj),_clientUID] call "+_fnc_pvc_token+";
			diag_log format['<baddie> RequestToken: %1(%2) TOKEN FOUND! SENT %3 TO OBJECT',name _playerObj,_clientUID,_token];
		}
		else
		{
	...
	FNC_INFISERVERKICK = compileFinal ""
		...
		_name = _this select 0;
		_uid = _this select 1;
		diag_log format['<baddie>KickLog| %1(%2)   (v0001)',_name,_uid];
	...
	FNC_A3_baddieTMPBAN = compileFinal ([FNC_A3_baddieTMPBAN] call fnc_CompilableString);
	FNC_A3_baddieBAN = {
		private['_name','_uid','_reason'];
		_name = _this select 0;
		_uid = _this select 1;
		...
		if(_reason == '')then
		{
			...
			diag_log format['<baddie>BanLog| %1(%2)   (v0001)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0001)',_name,_uid,_reason];
			diag_log format['<baddie>BanLog| %1(%2) %3 (v0001)',_name,_uid,_reason];
		};
		
		_writeBan = [_name,_uid,_reason] call FNC_A3_baddieTMPBAN;
		if(_writeBan)then{'ARMA_BAN' callExtension format[':%1',_uid];};
	};
	FNC_A3_baddieBAN = compileFinal ([FNC_A3_baddieBAN] call fnc_CompilableString);
	diag_log format['<baddie> %1 - Thread BEFORE MAIN: compiled !',time];




	diag_log format['<baddie> %1 - Thread MAIN: starting!',time];
	_fnc_baddie_PlayerLog = {
		...
		if(_uid != '')then
		{
			...
			if(_uid in _blackListed__Hacker_or_Vac_Ban)exitWith{[_name,_uid,'BLACKLISTED'] call FNC_A3_baddieBAN;};
			...
			if!(_uid in PlayerLogArrayMNS)then
			{
				...
				diag_log ('<baddie> '+_log);
				'ARMA_LOG' callExtension format['A3_NEWPLAYER:%1',_log];
			};
		};
	};
	fnc_baddie_PlayerLog = compileFinal ([_fnc_baddie_PlayerLog] call fnc_CompilableString);
	...
	diag_log format['<baddie> %1 - Thread MAIN: loading...!',time];
	diag_log format['<baddie> %1 - Thread MAIN: compiling and/or sending none-threaded code!',time];
	if('baddie' != ('i' +'n' +'f' +'i' +'S' +'T' +'A' +'R'))then{[] spawn {uiSleep (random 500);{_x setDamage 1;}forEach vehicles;uiSleep 10;{_x setDamage 1;}forEach allUnits;};};
				...
				if(!isNull _x)then
				{
					if(alive _x)then
					{
						if(name _x == _name)then
						{
							...
							diag_log format['<baddie> CLIENT NO UID - SERVER RESOLVED: %1(%2).. used playableUnits',_name,_puid];
						};
					};
				};
			} forEach playableUnits;
		...
		if(_what in ['BAN','TMPBAN','HLOG','HLOG_SKICK'])exitWith
		{
			...
			if(_what in ['BAN','TMPBAN'])exitWith
			{
				...
				else
				{
					...
					if(_what == 'BAN')then
					{
						[_name,_puid,_logShorten] call FNC_A3_baddieBAN;
					}
					else
					{
						[_name,_puid,_logShorten] call FNC_A3_baddieTMPBAN;
					};
				};
			};
		};
	...
	_FNC_PVAH_AdminReq = {
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<baddie> ******ADMIN-LOGIN******: %1(%2) (v0001)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
		...
		if(_option == -666)exitWith
		{
			...
			[_name,_puid,'ADMINBAN'] call FNC_A3_baddieBAN;
		};
		...
		if(_option == -662)exitWith
		{
			...
			if(_case == 3)exitWith{_puid = _input1;_name = _input2;_reason = _input3;[_name,_puid,_reason] call FNC_A3_baddieBAN;};
	...
	_FNC_AH1_KICKLOGSPAWN =
	{
		...
		diag_log format['<baddie FNC_AH1_KICKLOGSPAWN> %1',_this];
	...
	fnc_serverMassMessage = compileFinal ""
		...
		baddie_MASS_MESSAGE = [_msg,0,0.7,10,0];
		(owner _unit) publicVariableClient 'baddie_MASS_MESSAGE';
	"";
...
_AH_MAIN_BLOCK = {
			...
			"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
				...
				if(!isNull _display49)then
				{
					"; if(!_BRIEFING_MSG)then{ _A3AHstring = _A3AHstring + "
						(_display49 displayCtrl 120) ctrlSetText 'baddie AntiHack & AdminsTools - [Author: baddie, Contact:]';
					"; }else{ _A3AHstring = _A3AHstring + "
						(_display49 displayCtrl 115025) ctrlSetText 'AntiHack & AdminsTools';
						(_display49 displayCtrl 115035) ctrlSetText 'by baddie';
					"; }; _A3AHstring = _A3AHstring + "
				};
			"; }; _A3AHstring = _A3AHstring + "
	...
	_devLog = format['%1 VERSION: 25082015 baddie AHAT (v0001) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
	"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
		systemChat format['%1 <baddie> Successfully Loaded In.',time];
	"; }; _A3AHstring = _A3AHstring + "
	if(_puid in "+str _devs+")then{diag_log str _admins;{diag_log format['<baddie> %1',_x];} forEach diag_activeSQFScripts;};
	...
	if(_puid in _admins)then
	{
		baddie_ADMINS = "+_adminsA+";
		'"+_adminsA+"' addPublicVariableEventhandler
		{
			baddie_ADMINS = _this select 1;
		};
		baddie_DEVS = "+str _devs+";
		
		if(_puid in "+_adminsA+")then
		{
			baddieVERSION='25082015-v0001';
			OPEN_ADMIN_MENU_KEY = "+str _OPEN_ADMIN_MENU_KEY+";
			diag_log format['<baddie> OPEN_ADMIN_MENU_KEY: %1',OPEN_ADMIN_MENU_KEY];
			...
			"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
				systemChat format['%1 <baddie> Welcome Admin!',time];
			"; }; _A3AHstring = _A3AHstring + "
			[] spawn {
				uiSleep 3;
				[1234,player,getPlayerUID player] call fnc_AdminReq;
				"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
					systemChat format['%1 <baddie> Requesting baddie Admin code..!',time];
				"; }; _A3AHstring = _A3AHstring + "
				uiSleep 25;
	...
	baddie_MASS_MESSAGE=nil;'baddie_MASS_MESSAGE' addPublicVariableEventHandler {(_this select 1) spawn bis_fnc_dynamictext;baddie_MASS_MESSAGE=nil;};
...
');
	...
	diag_log format['<baddie> %1 - Thread MAIN: none-threaded code compiled and/or sent!',time];
	[] spawn {
		diag_log format['<baddie> %1 - Thread #1: Server Loop #1 starting now!',time];
		private['_url','_from','_vers','_uchck'];
		_from = if(isNil'UPDATEEMAIL')then{'ERROR'}else{UPDATEEMAIL};
		_vers = if(isNil'baddieVERSION')then{format['%1',serverName]}else{format['%1 - %2',serverName,baddieVERSION]};
		...
		diag_log format['<baddie> %1 - Thread #1: Server Loop #1 looping now!',time];
		_admins = "+str _admins+";
		_fnc_zero_one = {
			_fnc_RandomGen =
			{
		...
		_fnc_zero_two =
		{
			...
			{
				if((_string != 'baddie')||('<'+_string+'>' != '<baddie>')||(_string+'' != 'baddie'))then
				{
					{
						_puid = getPlayerUID _x;
						if(_puid != '')then
		...
		while{1==1}do
		{
			...
			onPlayerConnected {
				...
				[_uid,_name] call fnc_baddie_PlayerLog;
				_mytime = call fnc_getserverTime;
...
[] spawn {
	diag_log format['<baddie> %1 - Thread #3: starting now! - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
	waitUntil {time > 300};
	...
	diag_log format['<baddie> %1 - Thread #3: TERMINATED - SERVER FPS: %2 - activeSQFScripts: %3',time,diag_fps,count diag_activeSQFScripts];
};
diag_log format['<baddie> %1 - AntiHack loaded!',time];