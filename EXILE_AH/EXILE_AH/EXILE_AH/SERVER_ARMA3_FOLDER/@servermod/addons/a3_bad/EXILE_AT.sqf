...
baddie_MAIN_CODE = "
	_log = format['%1 <baddiebaddie> Loading Menu...',time];systemchat _log;diag_log _log;
	...
	if(isNil 'baddie_toggled_A')then{baddie_toggled_A = ['==== OnTarget ====','==== Toggleable ===='];};
	...
	_log = '<baddiebaddie> config data loaded...!';diag_log _log;
	fnc_setFocus = {
				...
				_fnc_search =
				{
					...
					switch (LASTSUBBUTTON) do {
						case 0:{
							...
							{
								_ctrl lbAdd '==== Weapons ====';
								if(!isNil 'baddie_add_weapons')then
								{
									{
							...
							if('==== Magazines ====' call ADMINLEVELACCESS)then
							{
								...
								if(!isNil 'baddie_add_magazines')then
								{
									{
						...
						case 1:{
							if('==== Vehicles ====' call ADMINLEVELACCESS)then
							{
								...
								if(!isNil 'baddie_add_vehicles')then
								{
									{
										_displayName = getText(configFile >> 'CfgVehicles' >> _x >> 'displayName');
										if(((toLower _x) find _txt > -1)||((toLower _displayName) find _txt > -1))then{
						...
						else
						{
							...
							lbclear _ctrl;
							{
								...
								if((toLower _name) find _txt > -1)then
								{
									...
									_PUIDX = getPlayerUID _x;
									if(((_PUIDX in baddie_ADMINS)&&!(_PUIDX in baddie_DEVS))||((_PUIDX in baddie_DEVS)&&(MYPUIDinfiESP in baddie_DEVS))||(_PUIDX == MYPUIDinfiESP))then{_clr = [0,1,0,1];};
									if(_x isEqualTo SELECTED_TARGET_PLAYER)then{_clr = [1,0.7,0.15,1];};
									_ctrl lbSetColor [_index,_clr];
								};
							} forEach (call fnc_baddie_get_LeftClicks);
						};
					};
					if(_txt == '')then
					{
						[] call fnc_fill_baddie_Player;
	...
	fnc_SearchFieldButtons = {
		...
		_btnItems buttonSetAction '
			LASTSUBBUTTON = 0;FILLMAINSTATE=1;[] call fnc_fill_baddie_MAIN;[] call fnc_setFocus;[] call fnc_colorButtons;
		';
		...
		_btnVehs buttonSetAction '
			LASTSUBBUTTON = 1;FILLMAINSTATE=1;[] call fnc_fill_baddie_MAIN;[] call fnc_setFocus;[] call fnc_colorButtons;
		';
		...
		_btnMisc buttonSetAction '
			LASTSUBBUTTON = 2;FILLMAINSTATE=1;[] call fnc_fill_baddie_MAIN;[] call fnc_setFocus;[] call fnc_colorButtons;
		';
	...
	fnc_Loadoutmenu = {
		...
		fnc_load_LoadOuts = {
			baddie_LoadOutARRay = profileNamespace getVariable['baddie_LoadOutARRay',[]];
			lbClear (findDisplay MAIN_DISPLAY_ID displayCtrl 44465);
			if!(baddie_LoadOutARRay isEqualTo [])then
			{
				{(findDisplay MAIN_DISPLAY_ID displayCtrl 44465) lbAdd _x;} forEach baddie_LoadOutARRay;
		...
		fnc_ButtonClick_44467 = {
			...
			if(_txt != '')then
			{
				if(_txt in baddie_LoadOutARRay)then
				{
					_id = baddie_LoadOutARRay find _txt;
					if(_id == -1)exitWith{systemchat 'can not find Loadout';};
					_return = baddie_LoadOutARRay deleteAt _id;
					systemchat format['Deleted Loadout: %1',_return];
					profileNamespace setVariable['baddie_LoadOutARRay',baddie_LoadOutARRay];saveprofileNamespace;
		...
		fnc_ButtonClick_44468 = {
			...
			if!(_txt in baddie_LoadOutARRay)then
			{
				baddie_LoadOutARRay pushBack _txt;
				profileNamespace setVariable['baddie_LoadOutARRay',baddie_LoadOutARRay];saveprofileNamespace;
			};
	...
	fnc_FULLinit = {
		disableSerialization;
		if(isNull findDisplay MAIN_DISPLAY_ID)then{createdialog 'baddie_AdminMenu';};
		[] call fnc_initMenu;
	...
	fnc_initMenu = {
		...
		FILLMAINSTATE=0;[] call fnc_fill_baddie_MAIN;
		...
		_ctrl ctrlSetText format['Players loaded in: %1 of %2               baddiebaddie   Admin Menu   SERVER UP FOR: %3h %4min               %5',count playableUnits,((playersNumber west)+(playersNumber east)+(playersNumber civilian)+(playersNumber resistance)),_hours,_minutes,baddieVERSION];
		
		_btnSortRange = _display displayCtrl 10;
		_btnSortRange buttonSetAction 'SortAlphaPlease = nil;SortRangePlease = true;[] call fnc_fill_baddie_Player;[] call fnc_setFocus;';
		
		_btnSortAlpha = _display displayCtrl 11;
		_btnSortAlpha buttonSetAction 'SortRangePlease = nil;SortAlphaPlease = true;[] call fnc_fill_baddie_Player;[] call fnc_setFocus;';
		
		_btnMainMenu = _display displayCtrl 20;
		_btnMainMenu buttonSetAction 'FILLMAINSTATE=0;[] call fnc_fill_baddie_MAIN;[] call fnc_setFocus;[] call fnc_colorButtons;';
		
		_btnAntiSpawnMenu = _display displayCtrl 21;
		_btnAntiSpawnMenu buttonSetAction 'FILLMAINSTATE=1;[] call fnc_fill_baddie_MAIN;[] call fnc_setFocus;[] call fnc_colorButtons;';
	...
	fnc_colorizeMain = {
		...
		for '_i' from 0 to _lsize do
		{
			...
			if(_lbtxt in baddie_Toggleable)then
			{
				if(_lbtxt in baddie_toggled_A)then
				{
			...
			if(_lbtxt in baddie_SubMenus)then
			{
				_ctrl lbSetColor [_i,[0.2,0.4,1,1]];
			};
			if(_lbtxt in baddie_OnTargetNICE)then
			{
				_ctrl lbSetColor [_i,[0,0.8,1,1]];
			};
			if(_lbtxt in baddie_OnTargetEVIL)then
			{
	...
	fnc_fill_baddie_MAIN = {
		...
		baddie_OnTarget = baddie_OnTargetNICE + baddie_OnTargetEVIL;
		baddie_Toggleable =
		[
			'baddie Player ESP','baddie AI ESP','baddie Dead ESP',
			'baddie MapIcons','Vehicle Marker','DeadPlayer Marker','Stealth / Invisible',
		...
		{
			_onTarget = [];
			{if(_x call ADMINLEVELACCESS)then{_onTarget pushBack _x;};} forEach baddie_OnTargetNICE;
			if!(_onTarget isEqualTo [])then{_onTarget pushBack '---';};
			{if(_x call ADMINLEVELACCESS)then{_onTarget pushBack _x;};} forEach baddie_OnTargetEVIL;
			if!(_onTarget isEqualTo [])then
			{
				_ctrl lbAdd '==== OnTarget ====';
				if(isNil 'baddie_add_OnTarget')then
				{
			...
			_Toggleable = [];
			{if(_x call ADMINLEVELACCESS)then{_Toggleable pushBack _x;};} forEach baddie_Toggleable;
			if!(_Toggleable isEqualTo [])then
			{
				_ctrl lbAdd '==== Toggleable ====';
				if(isNil 'baddie_add_Toggleable')then
				{
	...
	fnc_baddie_get_LeftClicks = {
		_array = playableUnits;
		_array append allDeadMen;
		_array
	};
	fnc_fill_baddie_Player = {
		disableSerialization;
		if(!isNil 'filling_baddie_Player')exitWith{};
		filling_baddie_Player = positionCameraToWorld [0,0,0];
		_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl LEFT_CTRL_ID;
		lbclear _ctrl;
		_unsorted = call fnc_baddie_get_LeftClicks;
		_sorted = _unsorted;
		lbclear _ctrl;
		if!(_sorted isEqualTo [])then
		{
			...
			{
				_sorted = [_unsorted,[],{filling_baddie_Player distance _x},'ASCEND'] call BIS_fnc_sortBy;
				{call _fnc_addPlayerToList} forEach _sorted;
			};
		};
		_display = findDisplay MAIN_DISPLAY_ID;
		...
		for '_i' from 0 to 10 do {_ctrl lbAdd '';};
		filling_baddie_Player = nil;
	};
	fnc_LBDblClick_LEFT = {
		_target = lbtext[LEFT_CTRL_ID,(lbCurSel LEFT_CTRL_ID)];
		
		if(_target == 'Sort by Range')then{SortAlphaPlease = nil;SortRangePlease = true;[] call fnc_fill_baddie_Player;[] call fnc_setFocus;};
		if(_target == 'Sort by Name')then{SortRangePlease = nil;SortAlphaPlease = true;[] call fnc_fill_baddie_Player;[] call fnc_setFocus;};
	...
	fnc_LBDblClick_RIGHT = {
		...
		if(_click in baddie_Toggleable)then
		{
			switch (_click) do {
				case 'baddie Player ESP':{call baddie_fnc_infiESP_player1;};
				case 'baddie AI ESP':{call baddie_fnc_infiESP_AI;};
				case 'baddie Dead ESP':{call baddie_fnc_infiESP_DEAD;};
				case 'baddie MapIcons':{call baddie_A3MAPICONS;};
				case 'Vehicle Marker':{call adminVehicleMarker;};
				case 'DeadPlayer Marker':{call adminDeadPlayer;};
				case 'God Mode':{call baddie_A3Invulnerability;};
				case 'Vehicle God Mode':{call fnc_baddie_A3cargod;};
				case 'Lower Terrain':{call fnc_LowerTerrain;};
				case 'Vehboost':{call baddie_VehicleBoost;};
				case 'UnlimAmmo':{[] spawn fnc_baddie_A3UnlAmmo;};
				case 'noRecoil':{[] spawn fnc_baddie_A3noRecoil;};
				case 'Stealth / Invisible':{call fnc_baddieHIDE;};
				case 'Disable Announces':{call fnc_DisableAnnouncements;};
				case 'Teleport In Facing Direction (10m steps)':{if(isNil'baddie_TpdirectionENABLED')then{baddie_TpdirectionENABLED=true}else{baddie_TpdirectionENABLED=nil;};};
			};
			if(_click in baddie_toggled_A)then
			{
				lbSetColor [RIGHT_CTRL_ID,1,[1,0,0,1]];
				baddie_toggled_A = baddie_toggled_A - [_click];
				format['%1 - 0',_click] call fnc_adminLog;
			}
			else
			{
				lbSetColor [RIGHT_CTRL_ID,1,[0,1,0,1]];
				baddie_toggled_A pushBack _click;
				format['%1 - 1',_click] call fnc_adminLog;
			};
		};
		if(_click in baddie_OnTarget)then
		{
			_target = lbtext[LEFT_CTRL_ID,(lbCurSel LEFT_CTRL_ID)];
			_uid = '';
			if(_target == '')then
		...
		switch (_click) do {
			case '==== OnTarget ====':{if(isNil 'baddie_add_OnTarget')then{baddie_add_OnTarget = true;} else {baddie_add_OnTarget = nil;};};
			case '==== Toggleable ====':{if(isNil 'baddie_add_Toggleable')then{baddie_add_Toggleable = true;} else {baddie_add_Toggleable = nil;};};
			case '==== Weapons ====':{if(isNil 'baddie_add_weapons')then{baddie_add_weapons = true;} else {baddie_add_weapons = nil;};};
			case '==== Magazines ====':{if(isNil 'baddie_add_magazines')then{baddie_add_magazines = true;} else {baddie_add_magazines = nil;};};
			case '==== Bags ====':{if(isNil 'baddie_add_bags')then{baddie_add_bags = true;} else {baddie_add_bags = nil;};};
			case '==== Vehicles ====':{if(isNil 'baddie_add_vehicles')then{baddie_add_vehicles = true;} else {baddie_add_vehicles = nil;};};
			...
			case 'Spawn Ammo':{[] call baddie_A3addAmmo;};
		...
		if((FILLMAINSTATE == 0)||(FILLMAINSTATE == 1))then{[] call fnc_fill_baddie_MAIN;};
	};
	fnc_LBSelChanged_RIGHT = {
		...
		if(_cfg != '')then
		{
			...
			if(_txt == '')then{_txt = 'by baddiebaddie';};
			['
				<t size=''0.4''>'+_txt+' </t>
	...
	baddie_A3Invulnerability = {
		...
		if(isNil 'baddie_A3GodRun')then{baddie_A3GodRun = 0;};
		if(baddie_A3GodRun == 0)then
		{
			baddie_A3GodRun = 1;
			baddie_A3InvulnerabilityLoop = [] spawn {
				while {1==1} do
				{
					if(baddie_A3GodRun == 0)exitWith{call fnc_A3_stopGod;};
		...
		else
		{
			baddie_A3GodRun = 0;
			call fnc_A3_stopGod;
			terminate baddie_A3InvulnerabilityLoop;
			call fnc_A3_stopGod;
	...
	baddie_fnc_infiESP_player1 = {
		if(isNil 'fnc_infiESP_statePlayer1')then{
			fnc_infiESP_statePlayer1 = 1;_log = 'baddie Player ESP - ON (ESP DECREASES FPS A LOT!)';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		} else {
			fnc_infiESP_statePlayer1 = nil;_log = 'baddie Player ESP - OFF';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		};
		call fnc_call_single_esps;
	};
	baddie_fnc_infiESP_AI = {
		if(isNil 'fnc_infiESP_stateAI')then{
			fnc_infiESP_stateAI = 1;_log = 'baddie AI ESP - ON (ESP DECREASES FPS A LOT!)';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		} else {
			fnc_infiESP_stateAI = nil;_log = 'baddie AI ESP - OFF';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		};
		call fnc_call_single_esps;
	};
	baddie_fnc_infiESP_DEAD = {
		if(isNil 'fnc_infiESP_stateDEAD')then{
			fnc_infiESP_stateDEAD = 1;_log = 'baddie DEAD ESP - ON (ESP DECREASES FPS A LOT!)';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		} else {
			fnc_infiESP_stateDEAD = nil;_log = 'baddie DEAD ESP - OFF';hint _log;cutText [format[_log], 'PLAIN DOWN'];
		};
		call fnc_call_single_esps;
	};
	fnc_draw_MapIcons = {
	...
	fnc_RscDisplayDebugPublic = {
		...
		waitUntil
		{
			...
			_title ctrlSetText 'Debug & Admin Console - modified by baddiebaddie';
	...
	[] spawn {
		...
		systemChat format['%1 <baddiebaddie> Menu Loaded - press F1 (default Key) to open it!',time];
		...
		fnc_infiAdminKeyDown = {
			...
			switch (_key) do {
				...
				case 0x05: {
					...
					{
						if('FlyUp' call ADMINLEVELACCESS)then{_handled=true;[0] call baddie_FlyUp;};
					};
					if(_ctrl)then
					{
						if('FlyUp' call ADMINLEVELACCESS)then{_handled=true;[1] call baddie_FlyUp;};
					};
				};
				case 0x06: {
					if(_shift)then{if('Teleport In Facing Direction (10m steps)' call ADMINLEVELACCESS)then{if(isNil'baddie_TpdirectionENABLED')exitWith{};_handled=true;[] call baddie_Tpdirection;};};
				};
				case 0x07: {
					if(_shift)then{if('EjectTarget' call ADMINLEVELACCESS)then{_handled=true;[] call baddie_Eject_Join;};};
				};
				case 0x43: {
				...
				case 0x2F: {
					if(ALLOW_ME_THIS_KEYBIND)then{
						if(_shift)then
						{
							_handled=true;[] call baddie_shortTP;
						};
					};
				};
				case 0x30: {
				...
				case 0x40: {
					if('HealSelf' call ADMINLEVELACCESS)then{[] call baddie_A3Heal;'HealSelf' call fnc_adminLog;};
				};
				case 0x41: {
					if('HealRepairNear' call ADMINLEVELACCESS)then{[] call baddie_A3RestoreNear;'HealRepairNear' call fnc_adminLog;};
				};
				case 0x0F: {
					if(_shift)then
					{
				...
				case 0x52: {
					if(ALLOW_ME_THIS_KEYBIND)then
					{
						[] execVM '\baddie_work_on_AH\TEST.sqf';
					};
				};
				case 83: {
					_handled = false;
				...
				case 0x57: {
					if('Spawn Ammo' call ADMINLEVELACCESS)then{[] call baddie_A3addAmmo;};
				};
				default {
					_handled = false;
				};
...
";
call compile (""+_MAIN_CODE+"=baddie_MAIN_CODE;baddie_MAIN_CODE=nil;");