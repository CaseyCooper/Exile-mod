...
*/
diag_log format['<infiSTAR.de> %1 - VERSION: 15022016 infiSTAR.de AHAT (v0032)',time];
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de';
UPDATEEMAIL='crftgaming@gmail.com';
INFISTARVERSION='15022016-(v0032)';
...
FNC_A3_LOG_RAND_VAR = compileFinal "
	format['A3_RANDOMVAR_LOG:%1 (v0032)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0032)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_RAND_VAR'] call MAR_fnc_log;};
";
FNC_A3_HACKLOG = compileFinal "
	format['A3_HACKLOG:%1 [FPS: %2|SERVERTHREADS: %3] (v0032)',_this,diag_fps,count diag_activeSQFScripts] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0032)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_HACKLOG'] call MAR_fnc_log;};
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	format['A3_SURVEILLANCELOG:%1 [FPS: %2|SERVERTHREADS: %3] (v0032)',_this,diag_fps,count diag_activeSQFScripts] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0032)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_SURVEILLANCELOG'] call MAR_fnc_log;};
";
FNC_A3_ADMINLOG = compileFinal "
	format['A3_ADMINLOG:%1 (v0032)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0032)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de__ADMINLOG'] call MAR_fnc_log;};
";
FNC_A3_CUSTOMLOG = compileFinal "
	format['A3_%1:%2 [FPS: %3|SERVERTHREADS: %4] (v0032)',_this select 0,_this select 1,diag_fps,count diag_activeSQFScripts] call FN_CALL_LOG_DLL;
	diag_log format['<infiSTAR.de>%1| %2 (v0032)',_this select 0,_this select 1];
...
FNC_A3_CONNECTLOG = compileFinal "
	format['A3_CONNECTLOG:%1 (v0032)',_this] call FN_CALL_LOG_DLL;
	diag_log format['<infiSTAR.de>ConnectLog| %1 (v0032)',_this];
";
FNC_A3_TOKENLOG = compileFinal "
	format['A3_TOKENLOG:%1 (v0032)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>TokenLog| '+_this+' (v0032)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	format['A3_RESOLVESTEAMNAMELOG:%1 (v0032)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0032)');
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['G','f','8','B','o','C','2','i','z','D','s','D','Z','o','M','Z','p','h','E','J','K','B','j','V','C','7','H','j','9','s'];
...
[
	...
	'SNI_PRZ_ZZZ_TargetPlayer','healit','O_fnc_ArmA','MLRN_EXEC','running_threads','catchemall1807','killtarget'
];
...
if!(_currentUniformPlayer isEqualTo _uniform)then
{
	...
	"; if(_log_uniform_and_vest_check_actions)then{ _A3AHstring = _A3AHstring + "
	_log = format['had bugged uniform (%1 should be %2), fixed!',_uniform,_currentUniformPlayer];
	[name player,getPlayerUID player,'SLOG',toArray(_log)] call "+_AHKickLog+";
...
if!(_currentVestPlayer isEqualTo _vest)then
{
	...
	"; if(_log_uniform_and_vest_check_actions)then{ _A3AHstring = _A3AHstring + "
	_log = format['had bugged vest (%1 should be %2), fixed!',_vest,_currentVestPlayer];
	[name player,getPlayerUID player,'SLOG',toArray(_log)] call "+_AHKickLog+";
	"; }; _A3AHstring = _A3AHstring + "
};
"";
		...
		[
			[_name,_uid,_AHKickOFF,_AHKickLog,_admin,_isNormal,_49code,_blacklistedAdminVariables,"+str _blacklistedVariables+",_tofindcontent,_tofindboth],
			{
				...
				_white = [
					...
					'avs_fnc_rearmturret','av8b_loadout_service'
				];
...
"; if(!_KYLE_MODE)then{ _A3AHstring = _A3AHstring + "
	...
	_iddchecks = {
		...
		"; if(_ESP)then{ _A3AHstring = _A3AHstring + "
		if(isNil'ExileClientSafeZoneESPEventHandler')then
		{
			if(_addCaseMEHold > -1)then
			{
				_addCaseMEH = addMissionEventHandler ['Draw3D', {}];
				_addCaseMEHold = _addCaseMEHold + 1;
				if(_addCaseMEHold != _addCaseMEH)then
				{
					_log = format['ESP CHECK #1:   %1 should be %2   (NEW EXPERIMENTAL CHECK)',_addCaseMEH,_addCaseMEHold];
					[_name,_puid,'SLOG',toArray(_log)] call _AHKickLog;
				};
			};
			...
			_addCaseMEH = addMissionEventHandler ['Draw3D',
			{
				if(!isNil'ExileHudEventHandle')then
				{
					...
					{
						_this call ExileClient_gui_hud_event_onDraw3d;
					};
				};
				if(!isNil'ExileClientSafeZoneESPEventHandler')then
				{
					20 call ExileClient_gui_safezone_safeESP;
				};
			}];
			_roundRandomNumberMEH = _roundRandomNumberMEH +1;
			if(_addCaseMEH isEqualTo _roundRandomNumberMEH)then
			{
				_addCaseMEHold = _addCaseMEH;
			}
			else
			{
				_log = format['ESP CHECK #2:   %1 should be %2   (NEW EXPERIMENTAL CHECK)',_addCaseMEH,_roundRandomNumberMEH];
				[_name,_puid,'SLOG',toArray(_log)] call _AHKickLog;
			};
		};
		"; }; _A3AHstring = _A3AHstring + "
	...
	if(_isNormal)then
	{
		...
		_bigInputArray pushBack [
			[_name,_uid,_AHKickOFF,_AHKickLog,_badIDDsToKick,_badIDDsToClose,_allowedIDDs,_iddchecks],
			{
				...
				_addCaseMEHold = -1;
				_gui_hud_event_onDraw3d = compile preprocessFileLineNumbers 'exile_client\code\ExileClient_gui_hud_event_onDraw3d.sqf';
				if!((str _gui_hud_event_onDraw3d) isEqualTo (str ExileClient_gui_hud_event_onDraw3d))then
				{
					_log = 'ExileClient_gui_hud_event_onDraw3d has been changed';
					[_name,_puid,'SLOG',toArray(_log)] call _AHKickLog;
				};
				_gui_safezone_safeESP = compile preprocessFileLineNumbers 'exile_client\code\ExileClient_gui_safezone_safeESP.sqf';
				if!((str _gui_safezone_safeESP) isEqualTo (str ExileClient_gui_safezone_safeESP))then
				{
					_log = 'ExileClient_gui_safezone_safeESP has been changed';
					[_name,_puid,'SLOG',toArray(_log)] call _AHKickLog;
				};
...
"; }; _A3AHstring = _A3AHstring + "
	...
	[
		[_admins],
		{
			...
			_devLog = format['%1 VERSION: 15022016 infiSTAR.de AHAT (v0032) - server running: %2:1790 - AdminNameTag: "+str _AdminNameTag+"',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
			"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
...
_fnc_infiSTAR_PlayerLog = {
	...
	[
		...
		'76561198041841141','76561198207891657','76561198257041725',
		'0'
	];
