...
_vests = ["V_Rangemaster_belt","V_TacVest_brn","V_BandollierB_cbr"];
_vest_item = _vests call BIS_fnc_selectRandom;

_zloot = ["Exile_Item_Catfood","Exile_Item_PlasticBottleFreshWater","Exile_Item_InstaDoc","Exile_Item_DuctTape","Exile_Item_JunkMetal","Exile_Item_LightBulb","Exile_Item_Rope","Exile_Item_CamoTentKit","Exile_Item_WoodPlank","Exile_Item_FuelCanisterFull"];
_zlootitem = _zloot call BIS_fnc_selectRandom;

_zombie addVest _vest_item; 
_zombie addItemToVest _zlootitem;
...
if ((_sound < 1) && (isnil "ryanzombiesdisableaggressive")) then {[_zombie] exec "\ryanzombies\soundsclose.sqf"}
_sound = _sound + 1
